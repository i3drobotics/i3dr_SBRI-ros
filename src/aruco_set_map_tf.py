...
class ArucoSetMap:
    ...
    def pub_map_scan(self):
      if (self.tf_ready):
        ...
        tf_msg.transform = self.map_scan_tf

        scale, shear, rpy, trans, perspective = tf.transformations.decompose_matrix(self.map_scan_tf)
        self.tfBroadcaster.sendTransform(trans, tf.transformations.quaternion_from_euler(rpy[0],rpy[1],rpy[2]), rospy.Time().now(), self.map_frame_id, self.scan_frame_id)

        #self.map_scan_tf_br.sendTransform(tf_msg)

    def get_aruco_scan_tf(self):
      #get tf from aruco to camera (scan)
      #tf = self.tfBuffer.lookup_transform(self.aruco_marker_frame_id, self.scan_frame_id, rospy.Time() )
      
      tf = self.tfListener.lookupTransform(self.aruco_marker_frame_id, self.scan_frame_id, rospy.Time())
      return tf

    def get_map_aruco_tf(self):
      #get tf from map to aruco
      #tf = self.tfBuffer.lookup_transform(self.map_frame_id, self.aruco_marker_frame_id, rospy.Time() )
      tf = self.tfListener.lookupTransform(self.map_frame_id, self.aruco_marker_frame_id, rospy.Time())
      return tf

    def calc_map_scan_tf(self,aruco_scan_tf,map_aruco_tf):
      T_ma_m = self.tfTransformer.fromTranslationRotation(map_aruco_tf[0], map_aruco_tf[1])
      T_as_m = self.tfTransformer.fromTranslationRotation(aruco_scan_tf[0], aruco_scan_tf[1])
